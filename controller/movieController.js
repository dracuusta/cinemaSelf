...
exports.movie_list = asyncHandler(async (req, res, next) => {
  let movies = await Movie.find({})
    ...
    .sort({ name: 1 })
    .exec();
  movies = Array.from(new Set(movies.map((movie) => movie.name))).map((name) =>
    movies.find((a) => a.name === name),
  );
...
exports.movie_create_get = asyncHandler(async (req, res, next) => {
  ...
  allDirectors = Array.from(new Set(allDirectors.map((d) => d.name))).map(
    (name) => allDirectors.find((d) => d.name === name),
  );
